...
remote-meeting-recorder/
  ...
  control-window.html           # 常時前面ミニ制御ウィンドウの中身（Phase 4・§14.3 参考）
  main.ts                       # 薄いエントリ
  src/
    ...
    ui/
      ...
      controlWindow.ts          # Electron BrowserWindow の常時前面ミニ制御（波形/停止・任意・Phase 4）→ control-window.html を loadFile + IPC
...
- **常時前面ミニ制御**: frameless/transparent/alwaysOnTop の `BrowserWindow`（`control-window.html`）。20 本バーの波形（`AnalyserNode` → IPC `audio-level`）、マイクミュート、停止。Obsidian のアクセントカラーに追従。
