...
>
> deferred（sysrec 拡張後に順次・§4.7）: 品質(bitrate) / 手動 Gain / Noise suppression / Voice isolation / 一時停止(pause) / system のライブメーター
...
- ライブ波形＝**マイク入力メーター**。source が mic/both のとき表示（sysrec のマイク捕捉とは別に concurrent でマイクを読む）。**system 単体のときはマイクを録っていないので波形は出さず**、経過時間＋「システム音声を録音中」表示にする（system の真のメーターは sysrec の RMS 出力拡張後・§4.7）。
- ファイルに効かせたい DSP（品質 / 手動ゲイン / ノイズ抑制 / 音声分離）は Web Audio 制約では録音物に乗らない → **sysrec.swift 拡張で対応**（§4.7）。「効くものから段階的」方針（決定事項）に従い、**未実装トグルは UI に出さない**。
...
仕上げ: sysrec normalize --in <file> --out <file>      # 単一ファイルの正規化（§4.5）
版数:   sysrec --version → {"abi":2,"version":"…"}     # 互換チェック（§4.5）
仕様:   sysrec dsp-spec → {agc:{…},norm:{…}}           # DSP 定数の申告（契約テスト用・§4.5）
...
DSP はバイナリ内蔵。プラグインは `--agc on|off` を渡すだけ。既定 on で **gated RMS -16 dBFS / ピーク -1 dBFS**（テストトーンで数値検証済み・[[Obsidian録音プラグイン向け 録音ノウハウレポート#4 レベル処理 DSP レシピ（実測検証済みの全定数）|レポート §4]]）。

**段の切り分け（Issue #4 で確定）** — 「レベルの自動調整」と「聞ける音量にする仕上げ」は別物として扱う。

| 段 | 掛かる条件 | 内容 |
|---|---|---|
| 録音時 AGC | `--agc on` のみ | -20 dBFS 目標・最大 +12 dB。時間方向のばらつきを均す |
| 録音時ノイズゲート | `--agc on` かつ `--mic-gate`/`--sys-gate` | 閾値未満を無音扱いで減衰 |
| 録音時リミッター | **常時** | 歪み防止。AGC とは別機能なので `--agc off` でも掛ける |
| 仕上げの正規化 | **常時**（mix / normalize） | -16 dBFS 目標・±18 dB クランプ。ミックス全体に単一ゲインなのでソース間バランスは変えない |
| 仕上げのリミッター | **常時** | ルックアヘッド・シーリング -1 dBFS |

- `--agc off`（手動ミキサー含む）は「録音時の自動調整をしない」であって「無補正で出す」ではない。**仕上げ段は必ず通す**。これを外していたため、AutoGain オフの録音が実質無音になっていた（Issue #4）。
- 正規化の測定は 400ms 窓 RMS を -42 dBFS でゲートするが、**全窓がゲート未満ならゲート無しで測り直す**（一番救済が要る小さい素材が無補正で落ちるのを防ぐ）。実質デジタル無音（-74 dBFS 未満）は素通し。
- single ソース（system のみ / mic のみ）は mix を通らないため、停止時に `sysrec normalize` で同じ仕上げを掛ける。こちらは**再サンプルしない**（入力のサンプルレート/チャンネル数を保つ）。
- 仕上げの正規化は **AutoGain の有無で掛け分けない**。録音時 AGC はゲート（-42 dBFS）未満に反応しないので「AGC オン＝補正済み」は成り立たない。掛け分けていた頃の実測では AutoGain **オン**の system 単独が**オフより 18 dB 小さく**、AutoGain を入れるほど小さくなる逆転が起きていた。無駄な再エンコードは、変更が ±1 dB 未満なら `normalize` が書き出しを省いて **exit 3** を返すことで防ぐ（呼び出し側は元ファイルを据え置く）。

#### 経路×補正の対応表（どの録音経路にどの補正が掛かるか・単一の真実の源）

| 経路 | 録音時 AGC | 手動ゲイン | 仕上げ正規化 | 仕上げリミッター |
|---|---|---|---|---|
| macOS both → mix | `--agc on` 時 | control ファイル | mix 内（常時） | mix 内（常時） |
| macOS single → 停止時 normalize | `--agc on` 時 | control ファイル | normalize（常時・±1dB 未満は exit 3 で省略） | normalize 内（常時） |
| macOS 片系 rescue → normalize | 同上 | 同上 | normalize（常時） | 同上 |
| Windows（取り込み時に全段） | AutoGain オン時（agcGain） | GainNode | normGain（常時・取り込み時） | DynamicsCompressor（常時） |

**この表が仕様**。経路を追加・変更するときは必ずここを更新し、全経路で「正規化とリミッターは常時」を守る。

#### DSP 定数の契約テスト（`sysrec dsp-spec`）

レベル処理の定数・ロジックは Swift（sysrec 本体）と TypeScript（Windows 経路 `src/recorder/agc.ts`）の**二重実装**であり、片側だけ変わる事故が実際に起きた。そこで:

- Swift 側の定数は `NormSpec` / `AGCProcessor` の static let に集約し、`sysrec dsp-spec` が JSON で申告する。
- E2E（`test/e2e.mjs` [14]）が実バイナリの `dsp-spec` と TS 定数を突き合わせて一致を検証する（実バイナリが無い環境ではスキップ）。
- **定数を変えるときは Swift と TS の両方を変える**。片方だけ変えると E2E が落ちる——それがこの仕組みの目的。

#### 実音声ゴールデンテスト（`npm run test:audio`）

fake-binary E2E は状態機械のみを検証し音声そのものを見ないため、レベル系の回帰（無音・小さい・逆転）を検出できなかった。`test/audio-matrix.sh` が実バイナリでトーンを録音して補完する（macOS ローカル専用・音量と既定出力を一時変更するため実行中はオーディオ使用不可）:

1. AutoGain × 手動ミキサー × source の 9 ケースで最終ファイルが -16 dBFS 近傍に収まること
2. 録音中の既定出力デバイス切替にシステム音の取り込みが追従すること（§4.6・第 2 出力デバイスが無ければスキップ）

リリース前にローカルで回す（CI は音声再生不可）。

### 4.6 デバイスライフサイクル（macOS）

システム音タップは「既定出力デバイスにぶら下げた集約デバイス」で駆動するが、**既定出力は録音中に変わる**（BT イヤホンの接続/切断・ユーザーの切替）。2026-07-24 に「録音中に BT イヤホンが切れて以降のシステム音が無音」という実害が出た。対策:

- sysrec が `kAudioHardwarePropertyDefaultOutputDevice` の変更を監視し、300ms デバウンス後に**集約デバイスと IOProc を作り直して追従**する（タップ自体は生かすので録音は継続・欠けは数百 ms）。失敗時は 1 秒後に 1 回再試行し、以降は次の変更イベントに任せる。
- 出力デバイスが変わるとタップのフォーマットも変わり得る（BT 44.1kHz ⇔ 内蔵 48kHz）ため、再構築時にフォーマットを読み直し、`Capture.handleSystem` は入力フォーマット変化を検知して変換器（FormatNormalizer）を作り直す。
- プラグイン側の**無音ウォッチ**: 録音開始から 10 秒、対象ソースのレベル（sysrec の level ファイル実測値）が 0 のままなら Notice で警告する（Windows の 5 秒ウォッチの macOS 版）。「録れているつもりが無音」を録り終わる前に気づけるようにする。

#### Windows の仕上げ正規化（取り込み時・`src/recorder/agc.ts`）

Windows は経路が別なので、同じ結果を別の方法で作る。

| | macOS | Windows |
|---|---|---|
| システム音の取り込み | Core Audio プロセスタップ | WASAPI ループバック（`getDisplayMedia` + `audio:'loopback'`） |
| **OS 出力音量の影響** | **受けない**（音量つまみより手前で分岐） | **受ける**（スピーカーへ送る最終ミックスを拾う） |
| 仕上げ正規化 | 停止後に `sysrec normalize` でファイルを測って書き戻す | **取り込み時**に静的ゲインで揃える |

保存時にやれない理由: MediaRecorder が最終ファイルを直接書くうえ、**Chromium は AAC をエンコードできない**（実測: `AudioEncoder.isConfigSupported` は `mp4a.40.2`=false / `opus`=true）。読み直して書き戻すには再エンコードが要るので、`.m4a` を保ったままの保存時正規化は不可能。ffmpeg.wasm 同梱は「ffmpeg 非依存」方針に反するため採らない。

そこで取り込み時に等価なことをする。出力音量による減衰は**録音中ほぼ一定**なので、累積測定から収束する**静的ゲイン**で正しく打ち消せる。

- グラフ: `source → gain(手動) → agcGain(AutoGain) → normGain(正規化) → limiter → dest`。macOS の 録音時AGC → normalize → リミッター と同じ並び。
- 測定は **AGC 適用後**のタップ（macOS の normalize が「AGC 済みの録音ファイル」を測るのと同じ位置）。
- 目標 -16 dBFS / ゲート -42 dBFS / ±18 dB クランプ ＝ macOS の `loudnessGain` と同じ定数。
- **AutoGain のオン/オフに関わらず常時**動かす（macOS の normalize が常時なのと同じ）。AGC が「時間方向のばらつき」を均すのに対し、こちらは「録音全体の音量」を目標へ寄せる。累積平均なので時間とともに動かなくなる＝静的ゲインとして振る舞う。
- 冒頭 1.5 秒ぶんの有音が溜まるまでゲインを動かさない（一瞬の物音で暴れないため）。**保存時方式と違い録り始めだけ収束途中になる**のが唯一の劣化点。
- ゲート超えが 10 秒溜まらない（＝素材全体が -42 dBFS 未満＝出力音量を絞りすぎた）ときはゲート無しで測り直す。macOS と同じ救済で、これが無いと一番救済が要る素材ほど無補正になる。

実機検証（Obsidian レンダラ・同一グラフ）: 入力 -25.2 dBFS → 出力 -15.5 dBFS / 入力 -30.8 dBFS → 出力 -15.5 dBFS。**入力が 5.6 dB 違っても出力は同一**＝出力音量から独立。

#### 版数ハンドシェイク（`sysrec --version`）

バイナリは main.js と**別配布**なので「本体だけ更新してバイナリが取り残される」事故が起きる。古いバイナリは知らない引数を黙って無視し、**0 バイトの出力**と**空のログ**を作るため、停止時に「録音ファイルが生成されませんでした」という原因の分からない形でしか表面化しなかった（doctor も存在だけ見て ok を出していた）。

- `sysrec --version` は `{"abi":N,"version":"x.y.z"}` を stdout に返す。`abi` は **CLI 契約のバージョン**で、契約を壊す変更で上げる（引数の追加だけなら据え置き）。
- プラグインは `REQUIRED_SYSREC_ABI` 以上を要求し、**録音開始前**と **doctor** の両方で照合する。古い版は `--version` を解さず非ゼロ終了するので「応答が無い＝古い」と判定できる。
- 非互換なら録音を**始めずに**、原因と「診断の『sysrec を取得』で入れ替える」直し方を出す。doctor では NG＋取得ボタンを出す。

### 4.7 sysrec 拡張ロードマップ（添付UI機能のうちバイナリ側が要るもの）
...
> Phase 1 の録音ビューは **Source / Save to / AGC / Format(M4A) / ライブ波形（マイク）** を備える。**Input 選択・Monitor** は Phase 4、**Noise suppression・Voice isolation・pause** は Phase 5（いずれも §4.7 の sysrec 拡張前提）。「効くものから段階的」方針に従い、未実装トグルは UI に出さない。
