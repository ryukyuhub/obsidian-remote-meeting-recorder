...
/** 状態ディレクトリ書き込みチェック（macOS/Windows 診断で共通）。 */
function stateDirCheck(ctx: RecorderContext): DoctorCheck {
...
/** curl -L --fail で dest にダウンロード（sysrec/whisper zip/モデルの各 fix で共通）。 */
function curlDownload(url: string, dest: string, timeoutMs: number): Promise<unknown> {
  return execFileAsync("curl", ["-L", "--fail", "-o", dest, url], { timeout: timeoutMs });
}
...
export function runDoctor(ctx: RecorderContext): DoctorCheck[] {
  ...
  const downloadFix: DoctorFix = {
    ...
    run: async () => {
      ...
      await curlDownload(url, sysrecTarget, DOWNLOAD_TIMEOUT_MS);
...
function transcribeChecks(ctx: RecorderContext): DoctorCheck[] {
  ...
  const winWhisperFix: DoctorFix = {
    ...
    run: async () => {
      ...
      await curlDownload(url, zip, ZIP_DOWNLOAD_TIMEOUT_MS);
      // Windows 10+ 同梱の tar.exe は zip を展開できる。
      await execFileAsync("tar", ["-xf", zip, "-C", dir], { timeout: EXTRACT_TIMEOUT_MS });
      try {
        fs.unlinkSync(zip);
      } catch {
        // 展開済みなので zip 削除失敗は無視
