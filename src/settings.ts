...
export class RMRSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    this.bindToggle(
      "音量の自動調整（AGC）",
      "録音中の音量を自動でそろえます（小さい声は最大 +12 dB まで持ち上げ、目標 -20 dBFS）。" +
        "聞き取り・文字起こしが安定します。オフにすると録れたままの音量バランスになります。" +
        "歪み防止のリミッター（-1 dBFS）はオフでも常に掛かります。" +
        "macOS では加えて、保存時に -16 dBFS へのラウドネス正規化も行います。",
      () => s.defaultAgc,
      (v) => (s.defaultAgc = v)
    );

    this.bindDropdown(
      "マイクのノイズゲート（無音カット・macOS のみ）",
    ...
    this.bindToggle(
      ...
      "録音ビューの初期モードを手動ミキサーにします。オンのとき AGC は無効になり、" +
        ...
        "フェーダーで作ったバランスは保存時もそのまま保たれます（仕上げの正規化は全体に同じゲインを" +
        "掛けるだけなので、ソース間のバランスは変わりません）。録音ごとに録音ビューで Auto / 手動を切り替えられます。",
      () => s.enableManualMixer,
